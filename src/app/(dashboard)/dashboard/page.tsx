    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <div className="rounded-xl border bg-card p-5 flex flex-col items-center gap-3">
          <ProgressRing value={understanding} size={80} strokeWidth={6} />
          <div className="text-center">
            <div className="text-sm font-medium">Understanding</div>
            {understandingHistory.length > 1 && (
              <div className="mt-1">
                <Sparkline data={understandingHistory} width={80} height={24} />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl border bg-card p-5 flex flex-col items-center gap-3">
          <ProgressRing value={retention} size={80} strokeWidth={6} />
          <div className="text-center">
            <div className="text-sm font-medium">Retention</div>
            {retentionHistory.length > 1 && (
              <div className="mt-1">
                <Sparkline data={retentionHistory} width={80} height={24} color="hsl(142, 71%, 45%)" />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl border bg-card p-5 text-center">
          ...
          <div className="mt-2 text-3xl font-bold">{quizzesTaken}</div>
        ...
        <div className="rounded-xl border bg-card p-5 text-center">
          ...
          <div className="mt-2 text-3xl font-bold">{topicsCovered}</div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-xl border bg-card p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/quizzes" className="text-xs text-primary hover:underline flex items-center gap-1">
          ...
          {recentAttempts.length === 0 ? (
            ...
            <div className="space-y-2">
              {recentAttempts.slice(0, 8).map((attempt) => (
                <div key={attempt.id} className="flex items-center justify-between rounded-lg border p-3 text-sm">
